...
if (numel(varargin)<3 || isempty(varargin{3}))
    folder = handles.root;
    handles.files = [dir([folder '*.jpg']); dir([folder '*.JPG']); dir([folder '*.png']); dir([folder '*.bmp'])];
else
...
for i=1:N-1
    ...
    values = unique(handles.sp_labels(sub2ind([xdim,ydim,handles.f], round(x),round(y),repmat(handles.f,size(x)))));
