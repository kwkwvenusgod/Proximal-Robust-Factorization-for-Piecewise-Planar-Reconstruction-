...
// --------------------------------------------------------------------------
// -- U_find_prev_border
// --   Updates the adjacency list for the previous label
// --------------------------------------------------------------------------
void IMG::U_find_prev_border()
{
   mexErrMsgTxt("Shouldn't be using this anymore!\n");
   /*if (prev_neighbors.isempty())
   {
      prev_neighbors.clear();
      prev_neighbors_scale.clear();
      arr( linkedList<int> ) temp_neighbors = allocate_memory< linkedList<int> >(prev_K);
      for (int i=0; i<N; i++)
      {
         int x = i%xdim;
         int y = i/xdim;
         unsigned int cur_label = prev_label[i];

         int llabel = (x>0 && prev_label[i]!=prev_label[i-1]) ? prev_label[i-1] : -1;
         int ulabel = (y>0 && prev_label[i]!=prev_label[i-xdim]) ? prev_label[i-xdim] : -1;
         int rlabel = (x<xdim-1 && prev_label[i]!=prev_label[i+1]) ? prev_label[i+1] : -1;
         int dlabel = (y<ydim-1 && prev_label[i]!=prev_label[i+xdim]) ? prev_label[i+xdim] : -1;

         if (llabel<cur_label) llabel = -1;
         if (ulabel<cur_label || ulabel==llabel) ulabel = -1;
         if (rlabel<cur_label || rlabel==llabel || rlabel==ulabel) rlabel = -1;
         if (dlabel<cur_label || dlabel==llabel || dlabel==ulabel || dlabel==rlabel) dlabel = -1;

         if (llabel>=0)
            if (temp_neighbors[cur_label].indexOf(llabel)<0)
               temp_neighbors[cur_label].addNodeEnd(llabel);
         if (ulabel>=0)
            if (temp_neighbors[cur_label].indexOf(ulabel)<0)
               temp_neighbors[cur_label].addNodeEnd(ulabel);
         if (rlabel>=0)
            if (temp_neighbors[cur_label].indexOf(rlabel)<0)
               temp_neighbors[cur_label].addNodeEnd(rlabel);
         if (dlabel>=0)
            if (temp_neighbors[cur_label].indexOf(dlabel)<0)
               temp_neighbors[cur_label].addNodeEnd(dlabel);
      }
      for (int k=0; k<prev_K; k++)
      {
         while (!(temp_neighbors[k].isempty()))
         {
            int neighbor = temp_neighbors[k].popFirst();
            prev_neighbors.addNodeEnd(std::pair<unsigned int, unsigned int>(k, neighbor));

            double scale = 0;
            for (int i=0; i<3; i++)
            {
               //scale += exp(-pow( (prev_app_mean[3*k+i] - prev_app_mean[3*neighbor+i]) / lambda_sigma, 2)/2);
               scale += -pow( (prev_app_mean[3*k+i] - prev_app_mean[3*neighbor+i]) / lambda_sigma, 2)/2;
            }
            scale = exp(scale);
            prev_neighbors_scale.addNodeEnd(scale);
         }
      }

      deallocate_memory(temp_neighbors);
   }*/
}
...
// 6. Moving Functions
//////////////////////////////////////////////////////////////////////
void IMG::Match_Gating(arr(double) pre_mean ,int SP_ind, linkedList<int> &gating, arr(double) thres){
    //find the SPs within the gating area around the hypothesis (proposed flow)
    arr(double) new_mean;
    for(int i=0;i<K;i++){
        if(i!=SP_ind && SP_arr[i]->N!=0){
            new_mean = SP_arr[i]->get_mean_pos();
            if(fabs(new_mean[0]-pre_mean[0])<=thres[0] && fabs(new_mean[1]-pre_mean[1])<=thres[1] ){
                gating.addNode(i);
            }
        }
    }
}
void IMG::Match_Gating(arr(double) pre_mean, int k,linkedList<int> &gating,arr(double) thres,arr(bool) neighbors){
    //find the SPs within the gating area around the hypothesis (proposed flow)
    linkedList<int> neighborsLL;
    U_find_border_SP(k,neighbors,neighborsLL);
    //printf("NB: %d,%d\n",k,neighborsLL.getLength());
    int nb_k;
    arr(double) new_mean;
    linkedListNode<int>* node = neighborsLL.getFirst();
    while (node != NULL){
        nb_k = node->getData();
        new_mean = SP_arr[nb_k]->get_mean_pos();
        if(SP_arr[nb_k]->N!=0){
            //printf("%d,%f,%f,%f\n",k,new_mean[0],pre_mean[0],fabs(new_mean[0]-pre_mean[0]));
            if(fabs(new_mean[0]-pre_mean[0])<=thres[0] && fabs(new_mean[1]-pre_mean[1])<=thres[1] ){
                gating.addNode(nb_k);
                Match_Gating(pre_mean, nb_k,gating,thres,neighbors);
            }
        }
        node = node->getNext();
        }
...
void IMG::Test_gating(int i,arr(double) thres){
         arr(bool) neighbors = allocate_memory<bool>(K,false);
         linkedList<int> gating;
         arr(double) new_mean;
            if(SP_arr[i]->N!=0){
                // lived
                gating.addNode(i);//can match to itself
                neighbors[i] = true;
                new_mean = SP_arr[i]->get_mean_pos();
                //printf("new: %f,%f\n",new_mean[0],new_mean[1]);
                //Match_Gating(new_mean, i,gating,thres,neighbors);//match to nbs within range
                Match_Gating(new_mean, i,gating,thres);//match to nbs within range
            }else{
                mexPrintf("empty SP\n");
             }

         linkedListNode<int>* node=gating.getFirst();
         PrintList<int>(gating);
...
/*if (index==0 || (index%xdim==31 && index/xdim==29))
{
   mexPrintf("%d \t %e, %e\n", new_k, prob, max_prob);
   //mexPrintf("\t\t%e, %e\n", temp->log_likelihood_test_point(data + index*5), temp->get_log_likelihood());
   //mexPrintf("\tapp: %e, %e\n", temp->log_likelihood_test_point_app(data+index*5), temp->get_log_likelihood_app());
   //mexPrintf("\tpos: %e, %e\n", temp->log_likelihood_test_point_pos(data+index*5), temp->get_log_likelihood_pos());
}*/

   //if (SP_arr[new_k]==NULL)
   //{
//      mexPrintf("%e, %e\n", prob, max_prob);
   //}
    //if(index==18216||index==18266){printf("del: %f,%d,%d,%d\n",prob,max_k,new_k);}
    //if(index==18216){printf("del: %f,%d,%d\n",prob,max_k,new_k);}
...
double IMG::move_switch_calc_delta(SP* &oldSP, SP* &newSP)
{
   ...
   // appearance is all the same
   double logprob = oldSP->log_likelihood_switch_app_prior(newSP);
   logprob -= oldSP->get_log_likelihood_app();
   double Ni = oldSP->get_N();

   // position gets tricky
   if (old_isold && new_isold) // relabeling old to dead
   {
      arr(double) mu_old = oldSP->get_mean_pos(true);
      arr(double) mu_new = newSP->get_mean_pos(true);
      arr(double) total = oldSP->get_total_pos();
      for (int d=0; d<2; d++)
      {
         double mu_newd = mu_new[d];
         double mu_oldd = mu_old[d];
         logprob += (mu_newd-mu_oldd)*total[d] - Ni/2*(mu_newd*mu_newd - mu_oldd*mu_oldd);
      }
   }
   else if (old_isold && !new_isold) // relabling old to new
   {
      logprob += U_calc_model_order(Ni, false);
      logprob -= U_calc_model_order(Ni, true);

      logprob += newSP->log_likelihood_test_merge_pos(oldSP) - newSP->get_log_likelihood_pos();

      arr(double) mu_old = oldSP->get_mean_pos(true);
      arr(double) total = oldSP->get_total_pos();
      arr(double) total2 = oldSP->get_total2_pos();
      for (int d=0; d<2; d++)
      {
         double mu_oldd = mu_old[d];
         logprob -= mu_oldd*total[d] - Ni/2*mu_oldd*mu_oldd - total2[d]/2;
      }
      logprob -= -Ni*1.0*1.837877066409;
   }
   else if (!old_isold && new_isold) // relabeling new to dead
   {
      logprob += U_calc_model_order(Ni, true);
      logprob -= U_calc_model_order(Ni, false);
      ...
      {
         double mu_newd = mu_new[d];
         logprob += mu_newd*total[d] - Ni/2*mu_newd*mu_newd - total2[d]/2;
      }
      logprob += -Ni*1.0*1.837877066409;

      logprob -= oldSP->get_log_likelihood_pos() - ( -1.0*1.837877066409 - oldSP->get_sumlogDelta_div2_pos() );
   }
   else
   {
      mexErrMsgTxt("Can not link a new to new!");
   }*/

   return logprob;

   /*double logprob = newSP->log_likelihood_test_merge(oldSP) + oldSP->get_log_likelihood_empty();
   logprob -= oldSP->get_log_likelihood() + newSP->get_log_likelihood_empty();

   logprob += U_calc_model_order(oldSP->get_N(), new_isold) + U_calc_model_order(0, old_isold);
   logprob -= U_calc_model_order(oldSP->get_N(), old_isold) + U_calc_model_order(0, new_isold);
...
bool IMG::move_switch_IMG()
{
   ...
   for (int k=0; k<K; k++)
   {
      if (!(SP_arr[k]->isempty()))
      {
         ...
         /*double a = move_switch_calc_delta(SP_arr[k], SP_new, SP_old[k], false);
         mexPrintf("1: %e\n", a);
         SP_arr[K] = new SP(new_pos, new_app, max_UID++);
         SP_arr[K]->merge_with(SP_arr[k], label, border_ptr, xdim, ydim);
         double b = move_switch_calc_delta(SP_arr[K], SP_arr[k], SP_old[K], SP_old[k]);
         mexPrintf("2: %e\n", b);
         K++;
         mexErrMsgTxt("done!\n");*/
         ...
         {
            /*arr(double) total_pos = SP_arr[k]->get_total_pos();
            arr(double) total_app = SP_arr[k]->get_total_app();
            int total_N = SP_arr[k]->get_N();
            arr(double) mean_pos = SP_arr[k]->get_mean_pos();
            arr(double) mean_app = SP_arr[k]->get_mean_app();
            mexPrintf("mean=%e,%e,%e,%e,%e\n", total_pos[0]/total_N, total_pos[1]/total_N, total_app[0]/total_N, total_app[2]/total_N, total_app[3]/total_N);
            mexPrintf("mu  =%e,%e,%e,%e,%e\n", mean_pos[0], mean_pos[1], mean_app[0], mean_app[2], mean_app[3]);*/

            //mexPrintf("ok=%d  k=%d  e=%e\n", k, best_k, best_energy);
//            if (best_k==182)
//               mexPrintf("k=%d  e=%e\n", best_k, best_energy);
...
bool IMG::move_local_IMG()
{
   ...
   for (int ki=0; ki<Nsp; ki++)
   {
      ...
      /*linkedListNode< pair<unsigned int, int> >* node = SP_arr[k]->neighbors.getFirst();
      mexPrintf("K=%d\t", k);
      while (node != NULL)
      {
         mexPrintf("(%d,%d)\t", node->getData().first, node->getData().second);
         node = node->getNext();
      }
      mexPrintf("\n");*/
...
//                  mexPrintf("===========================\n");
               ...
               /*if (index==0 || (x==31 && y==29))
               {
                  mexPrintf("%d\t%d\t%d\n", label[index-1-xdim], label[index-1], label[index-1+xdim]);
                  mexPrintf("%d\t%d\t%d\n", label[index-xdim], label[index], label[index+xdim]);
                  mexPrintf("%d\t%d\t%d\n", label[index+1-xdim], label[index+1], label[index+1+xdim]);

                  mexPrintf("max_k=%d  k=%d   max_prob=%e\n", max_k, k, max_prob);
                  mexPrintf("===========================\n");
               }*/
...
/*	prob += calc_log_label(SP1->get_N() + new_SP1->get_N(), alpha, epsilon);
	prob += calc_log_label(SP2->get_N() + new_SP2->get_N(), alpha, epsilon);
	prob -= calc_log_label(SP1->get_N() + new_SP1->get_N() + new_SP2->get_N(), alpha, epsilon);
	prob -= SP2->get_log_label();*/
//printf(",%f\n ",prob);
...
void IMG::move_refine_IMG()
{
   ...
   {
      int k = perm->data[ki];
      if (energies[k] < threshold)//SP_changed[k] && SP_arr[k]!=NULL && (SP_arr[k]->get_N()>split_thres))
      {
         //mexPrintf("Start: %d, %d, %d \n",K, k, SP_arr[k]->N);

          //printf("to split: %d,%d,%d\n",ki,k,K);
         //move_split_SP(k);
         debug=k;
        //if(k==378) {U_print_neighbors(k);break;}
         move_refine_SP(k);
         //mexPrintf("Done\n");
          //if(ki>40){ break;}
      }
...
void IMG::move_merge_SP_propose_region(int k,arr(bool) neighbors,linkedList<int> &check_labels,double &max_E,int &max_k)
{
   ...
   {
      neighborsLL.print();
      //SP_arr[debug]->borders.print();
      //SP_arr[k]->borders.print();
      mexPrintf("k=%d\n", debug);
      /*
      mexPrintf("%d\n", neighborsLL.getLength());
      mexPrintf("NeighborsLL:\n");
      neighborsLL.print();
      mexPrintf("PixelsLL:\n");
      SP_arr[k]->pixels.print();
      mexPrintf("BordersLL:\n");
      SP_arr[k]->borders.print();
      mexPrintf("%e\n", tmp_E);
      mexPrintf("%e\n", max_E);
      imagesc<unsigned int>(label,xdim,ydim, k-10, k+10);
      check_labels.print();
      waitforbuttonpress();
      */
...
void IMG::move_split_SP_propose(int index, int num_SP, int option, double &max_E,int &ksplit,int* new_ks)
{
   ...
   //if(index==252&&option==502) printf("2: prepare %d\n",broken);drawnow();
      if (broken)
      {
         max_E = -DBL_MAX;
         ksplit = -2;
   ...
   //if(index==252&&option==502) printf("3: prepare %d\n",broken);drawnow();
   ...
   //if(index==252&&option==502) printf("4: prepare \n");drawnow();
   /*
      // 3. local refinement
      //level 1: only refine the border between two SP
      //level 2: refine the borders between two SP and their neighbouring SPs
         int changed = 1;
      while(changed !=0){
         changed = 0;
         for( int i = 0; i < num_SP; i++ ){
            changed += move_local(K+i);
            printf("local: %d,%d,%d\n",i,K+i,changed);
         }
         K+=num_SP;
         delete SP_arr[index];
   ...
   */
      ...
      else
      {
         ...
         if (!SP_old[index] && !SP_old[option])
         {
            ...
            {
                /*
               if ((index==177 && option==493) || (index==493 && option==177))
               {
                  mexPrintf("=========================\n");
                  mexPrintf("SP1_mean_pos[d] = %e\n", SP1_total_pos[d]/N1);
                  mexPrintf("SP2_mean_pos[d] = %e\n", SP2_total_pos[d]/N2);
                  mexPrintf("SPK1_mean[d] = %e\n", SPK1_total_pos[d]/NK1);
                  mexPrintf("SPK2_mean[d] = %e\n", SPK2_total_pos[d]/NK2);
                  drawnow();
               }
                */
...
mexPrintf("index=%d\noption=%d\nK=%d\nK+1=%d\n", index, option, K, K+1);*/
            ...
            if ((index==183 || index==504) && (option==183 || option==504))
            {
               mexPrintf("E1K1=%e\n", E_1_K1);
               mexPrintf("E1K2=%e\n", E_1_K2);
               arr(double) temp;
               double tempN;
               temp = SP_arr[index]->get_mean_pos();
               mexPrintf("index : (%e,%e)\n", temp[0], temp[1]);
               temp = SP_arr[option]->get_mean_pos();
               mexPrintf("option : (%e,%e)\n", temp[0], temp[1]);
               temp = SP_arr[K]->get_total_pos();
               tempN = SP_arr[K]->get_N();
               mexPrintf("K : (%e,%e)\n", temp[0]/tempN, temp[1]/tempN);
               temp = SP_arr[K+1]->get_total_pos();
               tempN = SP_arr[K+1]->get_N();
               mexPrintf("K+1 : (%e,%e)\n", temp[0]/tempN, temp[1]/tempN);
            }
            if (E_1_K1 > E_1_K2)
            {
...
void IMG::move_split_SP(int index)
{
   ...
   if (SP_arr[index]!=NULL && !(SP_arr[index]->isempty()) && SP_arr[index]->get_N() > num_SP)
   {
      ...
      for (int i=0; i<N; i++)
      {
         if (label[i]==index) nindex++;
         if (label[i]==new_ks[0]) nk1++;
         if (label[i]==new_ks[1]) nk2++;
         if (label[i]==ksplit) nksplit++;
      }

      if ((SP_arr[index]==NULL && nindex !=0) || (SP_arr[index]!=NULL && SP_arr[index]->get_N()!=nindex))
      {
         mexPrintf("ERROR: k=%d %d\n", index, nindex);
         mexErrMsgTxt("ERROR\n");
      }
      if ((SP_arr[new_ks[0]]==NULL && nk1!=0) || (SP_arr[new_ks[0]]!=NULL && SP_arr[new_ks[0]]->get_N()!=nk1))
      {
         mexPrintf("ERROR: k=%d %d\n", new_ks[0], nk1);
         mexErrMsgTxt("ERROR\n");
      }
      if ((SP_arr[new_ks[1]]==NULL && nk2!=0) || (SP_arr[new_ks[1]]!=NULL && SP_arr[new_ks[1]]->get_N()!=nk2))
      {
         mexPrintf("ERROR: k=%d %d\n", new_ks[1], nk2);
         mexErrMsgTxt("ERROR\n");
      }
      if (ksplit>=0 && ((SP_arr[ksplit]==NULL && nksplit!=0) || (SP_arr[ksplit]!=NULL && SP_arr[ksplit]->get_N()!=nksplit)))
      {
         mexPrintf("ERROR: k=%d %d\n", ksplit, nksplit);
         mexErrMsgTxt("ERROR\n");
      }*/
...
void IMG::move_refine_SP(int index)
{
   linkedList< std::pair<int, int> > neighbors(SP_arr[index]->neighbors);
   linkedListNode< std::pair<int, int> >* node = neighbors.getFirst();
   int tmp;
   bool done=false;
   while (node!=NULL && !done)
   {
      tmp = node->getData().first;
      //if(index==252) printf("hello :%d\n",tmp);
      done = move_refine_SP(index, tmp);
      //if(done&&tmp==243) {mexPrintf("ola: %d\n",index);mexErrMsgTxt("hoho");}
      node = node->getNext();
   }
   //if(index==252) printf("done\n");
}
bool IMG::move_refine_SP(int index1, int index2)
{
   /*if ((index1==183 || index1==504) && (index2==183 || index2==504))
   {
      mexPrintf("0 ===========================\n");
      arr(double) temp;
      double tempN;
      temp = SP_arr[index1]->get_mean_pos();
      mexPrintf("index1 %d : (%e,%e)\n", SP_arr[index1]->isold(), temp[0], temp[1]);
      temp = SP_arr[index2]->get_mean_pos();
      mexPrintf("index2 %d : (%e,%e)\n", SP_arr[index2]->isold(), temp[0], temp[1]);
   }*/

   bool done = false;
   double merge_E ,split_E= -DBL_MAX;
   int num_SP = 2;
   int *new_ks = new int[num_SP];
   memset(new_ks,-1,sizeof(int)*num_SP);

   // 1. merge them
   merge_E = move_merge_calc_delta(index1, index2);
   // 1.1 back-up SP
   // will mess around index1 and index2
   SP_arr[K] = new SP(*SP_arr[index1]);
   SP_arr[K+1] = new SP(*SP_arr[index2]);
   ...
   /*if ((index1==183 || index1==504) && (index2==183 || index2==504))
   {
      ...
      arr(double) temp;
      double tempN;
      temp = SP_arr[K]->get_mean_pos();
      mexPrintf("index1 %d : (%e,%e)\n", SP_arr[K]->isold(), temp[0], temp[1]);
      temp = SP_arr[K+1]->get_mean_pos();
      mexPrintf("index2 %d : (%e,%e)\n", SP_arr[K+1]->isold(), temp[0], temp[1]);
   }*/

   //linkedList<int> L1 = linkedList<int>(SP_arr[index1]->pixels);
   //linkedList<int> L2 = linkedList<int>(SP_arr[index2]->pixels);

   // 1.1 update label
   linkedListNode<int>* node = SP_arr[index2]->pixels.getFirst();
   while (node != NULL)
   {
      label[node->getData()] = index1;
      node = node->getNext();
   }
   //mexPrintf("before: %d,%d,%d,%d,%d  ",K, SP_arr[index1]->N,SP_arr[index2]->N, SP_arr[K]->N,SP_arr[K+1]->N);

   SP_arr[index1]->merge_with(SP_arr[index2], label, border_ptr, xdim, ydim);
   K += 2;
   //if(index1==252&&index2==502) printf("1: prepare\n");
   ...
   /*if ((index1==183 || index1==504) && (index2==183 || index2==504))
   {
      ...
      temp = SP_arr[K-2]->get_mean_pos();
      mexPrintf("K+0 %d : (%e,%e)\n", SP_arr[K-2]->isold(), temp[0], temp[1]);
      temp = SP_arr[K-1]->get_mean_pos();
      mexPrintf("K+1 %d : (%e,%e)\n", SP_arr[K-1]->isold(), temp[0], temp[1]);
   }*/

   // 2. split index1
   move_split_SP_propose(index1, num_SP, index2, split_E, num_SP, new_ks);
   //mexPrintf("sp: %d,%d,%f,%f\n",index1,index2,split_E,merge_E);drawnow();
   //if(index1==252&&index2==502) {U_print_neighbors(new_ks[0]);U_print_neighbors(new_ks[1]);}
   if(split_E+merge_E>0)
   {
/*if ((index1==183) || (index2==183))
{
   mexPrintf("HERE1 ===========================\n");
   arr(double) temp;
   double tempN;
   temp = SP_arr[index1]->get_mean_pos();
   mexPrintf("index1 %d %d : (%e,%e)\n", index1, SP_arr[index1]->isold(), temp[0], temp[1]);
   temp = SP_arr[index2]->get_mean_pos();
   mexPrintf("index2 %d %d : (%e,%e)\n", index2, SP_arr[index2]->isold(), temp[0], temp[1]);

      temp = SP_arr[K-2]->get_mean_pos();
      mexPrintf("K+0 %d %d : (%e,%e)\n", K-2, SP_arr[K-2]->isold(), temp[0], temp[1]);
      temp = SP_arr[K-1]->get_mean_pos();
      mexPrintf("K+1 %d %d : (%e,%e)\n", K-1, SP_arr[K-1]->isold(), temp[0], temp[1]);

      mexPrintf("new_ks[0]=%d\n", new_ks[0]);
      mexPrintf("new_ks[1]=%d\n", new_ks[1]);
}*/
      done = true;
      // update the label
      linkedListNode<int>* node = SP_arr[new_ks[0]]->pixels.getFirst();
      ...
      SP_arr[index1]->empty();
      SP_arr[index2]->empty();
      SP_arr[index1]->merge_with(SP_arr[new_ks[0]], label, border_ptr, xdim, ydim);
      SP_arr[index2]->merge_with(SP_arr[new_ks[1]], label, border_ptr, xdim, ydim);
      SP_arr[new_ks[0]]->empty();
      SP_arr[new_ks[1]]->empty();
      delete SP_arr[new_ks[0]];
      delete SP_arr[new_ks[1]];
      ...
      /*delete SP_arr[index1], SP_arr[index2];
      SP_arr[index1] = SP_arr[new_ks[0]];
      SP_arr[index2] = SP_arr[new_ks[1]];
      //if(index2==243) {U_print_neighbors(index1);U_print_neighbors(index2);}
      SP_arr[new_ks[0]] = NULL;
      SP_arr[new_ks[1]] = NULL;*/

      //delete SP_arr[index] backup
      SP_arr[K-1]->empty();
      SP_arr[K-2]->empty();
      delete SP_arr[K-1];
      delete SP_arr[K-2];
      //need to exchange new_ks[0],new_ks[1] with index1,index2
      // now update the neighbors list
      U_fix_neighbors_self(index1);
      U_fix_neighbors_self(index2);
      // update the neighbors' neighbors
      U_fix_neighbors_neighbors(index1, index2);
      U_fix_neighbors_neighbors(index2, index1);
      //if(index2==243) {U_print_neighbors(index1);U_print_neighbors(index2);}

      SP_changed[index1] = true;
      SP_changed[index2] = true;
   }
   else
   {
      //mexPrintf("back...%d,%d  ",SP_arr[index1]->N,SP_arr[index2]->N);
      // previous SP
      //SP_changed[index1] = false;
      delete SP_arr[index1];
      delete SP_arr[index2];
      SP_arr[index1] = SP_arr[K-2];
      SP_arr[index2] = SP_arr[K-1];

      // previous label
      int tmp_ind;

      // first change the labels
      //linkedListNode<int> *node = L1.getFirst();
      linkedListNode<int> *node = SP_arr[index1]->pixels.getFirst();
      while (node!=NULL)
      {
         label[node->getData()] = index1;
         node = node->getNext();
      ...
      //node = L2.getFirst();
      node = SP_arr[index2]->pixels.getFirst();
      while (node!=NULL)
      {
         label[node->getData()] = index2;
         node = node->getNext();
      ...
      /*
      // now update the actual pixel lists
      node = L1.getFirst();
      while (node!=NULL)
      {
         tmp_ind = node->getData();
         SP_arr[index1]->add_pixel(data, tmp_ind, U_check_border_pix(tmp_ind), pixel_ptr[tmp_ind], border_ptr[tmp_ind]);
         node = node->getNext();
      }
      node = L2.getFirst();
      while (node!=NULL)
      {
         tmp_ind = node->getData();
         SP_arr[index2]->add_pixel(data, tmp_ind, U_check_border_pix(tmp_ind), pixel_ptr[tmp_ind], border_ptr[tmp_ind]);
         node = node->getNext();
      }
      mexPrintf("ok...%d,%d\n",SP_arr[index1]->N,SP_arr[index2]->N);

      */

      //delete extra splitted
      for( int i = 0; i < num_SP; i++ )
      {
         //mexPrintf("wo: %d,%d\n",K+i,SP_arr[K+i]->N);
         //mexPrintf("pp: %d\n", SP_arr[K+i]->pixels.getLength());
         SP_arr[K+i]->empty();
         delete SP_arr[K+i];
         SP_arr[K+i] = NULL;
      }
...
bool IMG::U_Kmeans_plusplus(int index, int *bbox, int num_SP, int numiter, int index2)
{
   ...
   /*//calculate the distance to the first seed and sample the following seeds proportional to it
   for( int i = 1; i < num_SP; i++ )
   {
      ...
      {
         if (distvec[n]!=0){
            //tmp_dist = U_dist(seed,SP_arr[index].pixels(n));
            tmp_dist = U_dist(seed,node->getData());
            if(tmp_dist==0){
               //won't be selected any more
               distvec[n] = 0;
            }else{
               //additional weights
               distvec[n] += tmp_dist;
...
/*
    if(debug==378){
        mexPrintf("wala: %d,%d\n",K,label_count);
      imagesc<unsigned int>(label,xdim,ydim, K-1, K+label_count);
      waitforbuttonpress();
    }
*/
