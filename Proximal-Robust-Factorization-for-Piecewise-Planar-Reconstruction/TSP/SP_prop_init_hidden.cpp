...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
   ...
   checkInput(prhs, 7, MATRIX ,  "int32"); //hidden_count
   checkInput(prhs, 8, MATRIX3 , "int32"); //hidden_label
   ...
   arr(int) hidden_count = getArrayInput<int>(prhs[7]);
   arr(int) hidden_label = getArrayInput<int>(prhs[8]);
   const int *dims = mxGetDimensions(prhs[8]);
   int D;
   ...
   for (int x=0; x<X; x++) for (int y=0; y<Y; y++)
   {
      ...
      int count = 0;
      int maxCount = hidden_count[index];
      for (int d=0; d<D; d++)
      {
         int k = hidden_label[index+d*N];
         if (k>=0)
         {
            count++;
            int newx = x + vx[k] + 0.5;
