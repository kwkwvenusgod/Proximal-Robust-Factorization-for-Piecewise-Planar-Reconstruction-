%test script for running PARSuMi without missing data
% Just take mask = tru(m,n) and pass into the same function.
% basically a proximal regularized DRMF
...
rate=0.5;
W=rand(m,r)*rand(r,n);
%Using cases without missing data
mask=true(m,n);%rand(m,n)<rate;
sigma=0.01;
Z=sigma*randn(m,n);
E=2*(rand(m,n)-0.5);
orate=0.1;
...
% uncomment the following to try random initialization
% E0=zeros(size(E));
% Nini=randn(m,r);
