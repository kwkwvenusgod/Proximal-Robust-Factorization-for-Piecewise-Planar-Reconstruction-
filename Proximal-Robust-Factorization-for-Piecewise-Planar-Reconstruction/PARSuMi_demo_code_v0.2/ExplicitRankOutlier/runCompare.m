...
% Wcap=data_matrix(1:24,1:50);
% Wcap=data_matrix(31:60,100:300);
% Wcap=data_matrix(1:42,1:180); %Largest data matrix possible with explicit
% rank
% Wcap=data_matrix(1:42,1:160); %Good results
% Wcap=data_matrix(1:42,1:130); %Large data matrix with good convergence speed
sub_m = 42;
sub_n = 130;
Wcap=data_matrix(1:sub_m,1:sub_n); %Large data matrix with good convergence speed
[m, n]= size(Wcap);
...
if data_ok == 1,
    ...
    [U2,V2] = wiberg_nomean(Wcap_outlier, H, r, Vini);
...
fprintf('Sub data matrix used: %d x %d\n',sub_m,sub_n);
fprintf('\nFrobenious norm of error for Wiberg L2 = %.6f \n\n',res);
res2=norm(sample_mask.*(Wr-Wcap),'fro');
fprintf('\nFrobenious norm of error for explicit rank = %.6f \n\n',res2);
