function [W_bar,residue_X,n_iter] = APG_MC(Wcap,sample_mask,W_0,gamma_W)
...
for i=1:max_iter      
    ...
    %Gradient evaluated at (Y_W_k) 
    grad_f_Y_W_k = ATAop((Y_W_k - Wcap),sample_mask);
...
%     grad_f_Y_E_k = ATAop((W_k  - Wcap),sample_mask);
% 
%     % Optimize over E
%     G_E = Y_E_k - grad_f_Y_E_k;     
%     E_kplus1 = L1Minimizer(G_E,gamma_E); 
    
    [has_converged,residue_X(i)] = CheckConvergence(Wcap,sample_mask,W_kplus1,Y_W_k,tol);    
...
function [flag,residue] = CheckConvergence(Wcap,sample_mask,W_kplus1,Y_W_k,tol)

flag = false;
grad_f_kplus1 = ATAop((W_kplus1  - Wcap),sample_mask);
grad_f_Y_k = ATAop((Y_W_k - Wcap),sample_mask);
S_W = (Y_W_k - W_kplus1) + grad_f_kplus1 - grad_f_Y_k;
S_kplus1 = S_W ;
residue = norm(S_kplus1,'fro') / max(norm([W_kplus1],'fro'),1);
